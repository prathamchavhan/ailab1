...
//   useEffect(() => {
//     const fetchAptitudeLeaderboard = async () => {
//       setIsLoading(true);
...
//       const { data, error } = await supabase.rpc('get_average_aptitude_leaderboard', {
//         filter_mode: activeTab.toLowerCase(), // 'inter' or 'intra'
//         user_id_param: user.id               // The current user's ID
//       });
...
//     fetchAptitudeLeaderboard();
//   }, [supabase, activeTab]); // Re-run this effect when activeTab changes

//   return (
//     <div className="bg-transparent p-6 rounded-lg w-full max-w-md">
//       <p className="font-bold text-[#09407F] text-[20px] mb-4">Aptitude Leaderboard</p>
...
//                       <img 
//                         // FIX 1: Use user.display_name for the alt text and avatar fallback
//                         src={user.avatar_url || `https://ui-avatars.com/api/?name=${user.display_name || 'User'}&background=random`} 
//                         alt={user.display_name || 'User'}
//                         className="w-8 h-8 rounded-full object-cover"
//                       />
//                       {/* FIX 2: Use user.display_name for the user's name */}
//                       <span className="font-medium text-sm text-gray-700">{user.display_name}</span>
//                     </div>
//                   </td>
//                   {/* Rank Cell */}
...
//                   </td>
//                   {/* Score Cell */}
//                   {/* FIX 3: Use user.average_score for the score */}
...
export default function Leaderboard() {
  ...
  useEffect(() => {
    const fetchAptitudeLeaderboard = async () => {
      ...
      const { data, error } = await supabase.rpc('get_average_aptitude_leaderboard1', {
