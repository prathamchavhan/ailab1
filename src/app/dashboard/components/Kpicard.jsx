// // File: components/Kpicard.js (Unchanged)
...
// File: components/Kpicard.js (Modified)
...
function DetailedScoreCard({
  ...
  const renderMetrics = () => {
    ...
    // --- MODIFICATION FOR 'interview' TYPE ---
    // This is the logic that runs for type === "interview"
    
    // 1. Define the specific metrics we want to show (in lowercase for a safe, case-insensitive check)
    const allowedInterviewMetrics = [
      "technical skill",
      "problem solving",
      "communication",
    ...
    // 2. Filter the scores.metrics object
    const interviewMetrics = Object.entries(scores.metrics).filter(
      ([label]) => allowedInterviewMetrics.includes(label.toLowerCase())
    );

    // 3. Render only those filtered metrics
    return interviewMetrics
      .sort(([labelA], [labelB]) => labelA.localeCompare(labelB))
      .map(([label, score]) => (
        <MetricItem
          key={label}
          score={score}
          label={label} // Use the original label (with capitalization) for display
          color={themeColor}
        />
      ));
    // --- END OF MODIFICATION ---
